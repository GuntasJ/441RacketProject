...
;This also makes the main parsing code much cleaner and easier to read. It reads basically like english.
...
(define (semi-colon-e? token-list)
  (cond
    [(empty? token-list) (failure (~a "Syntax Error at line " nl-count))]
    [(equal? (first (first token-list)) 'SEMI_COLON) (success (rest token-list))]
    [else (error (~a "Syntax Error at line " (rest (first token-list))))]))
...
(define (is-expression token-list)
  ...
  (cond
    ...
    [else (if (empty? token-list)
              (failure (~a "Syntax Error at line " nl-count))
              (failure (~a "Syntax Error at line " (rest (first token-list)))))]))
...
;All of the functinos in this right hand side chain will throw errors, and not failure. This is because they can contain multiple statements, which results in loss information when failure is going up.

(define (is-statement token-list)
  (displayln (~a "is-statement: " token-list))
  (cond
    [(success? (id? token-list)) (chain semi-colon-e? (chain is-expression (=-e? (rest token-list))))]
    [(success? (if? token-list)) (chain semi-colon-e? (chain is-statement (chain rp-e? (chain is-boolean (lp-e? (rest token-list))))))]
    [(success? (while? token-list)) (chain semi-colon-e? (chain endwhile-e? (chain is-line-list (chain rp-e? (chain is-boolean (lp-e? (rest token-list)))))))]
    [(success? (read? token-list)) (chain semi-colon-e? (id-e? (rest token-list)))]
    [(success? (write? token-list)) (chain semi-colon-e? (is-expression (rest token-list)))]
    [(success? (goto? token-list)) (chain semi-colon-e? (id-e? (rest token-list)))]
    [(success? (gosub? token-list)) (chain semi-colon-e? (id-e? (rest token-list)))]
    [(success? (return? token-list)) (semi-colon-e? (rest token-list))]
    [(success? (break? token-list)) (semi-colon-e? (rest token-list))]
    [(success? (end? token-list)) (semi-colon-e? (rest token-list))]
    [else (if (empty? token-list)
              (failure (~a "Syntax Error at line " nl-count))
              (failure (~a "Syntax Error at line " (rest (first token-list)))))]))
...
(define (is-line token-list)
  ...
  (cond
    [(success? (is-label token-list)) (chain is-line-tail (chain is-statement (is-label token-list)))]
    [else (failure "idk")]))
...
(define (parse-string input-string)
  ...
  (with-handlers ([exn:fail? (λ (e) (exn-message e))])
    ...
    (if (failure? end-tokens)
        (from-either end-tokens)
        (if (chain empty? end-tokens)
            "Accept"
            (~a "Syntax Error at line " (rest (chain first end-tokens)))))))
